...
import { ArrowRight, FileText, AlertTriangle, Shield, BarChart3, Users, ChevronRight } from 'lucide-react';
  ...
  return (
    <>
      ...
      <section className="relative bg-gradient-to-br from-blue-950 via-blue-900 to-blue-800 overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-amber-500/10 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl" />
        <div className="absolute bottom-0 left-0 w-72 h-72 bg-blue-400/10 rounded-full translate-y-1/2 -translate-x-1/3 blur-3xl" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28 lg:py-36">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
              Transparencia y{' '}
              <span className="text-amber-400">Control Fiscal</span>
            </h1>
            <p className="text-lg sm:text-xl text-blue-200 leading-relaxed mb-10 max-w-2xl">
              Órgano de control fiscal del Estado Mérida, comprometido con la vigilancia de los recursos públicos, la rendición de cuentas y el fortalecimiento de la gestión pública.
            </p>
      ...
      <section className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { icon: Shield, label: 'Control Fiscal', desc: 'Vigilancia y fiscalización', to: '/competencias', color: 'bg-blue-600' },
              { icon: FileText, label: 'Marco Legal', desc: 'Gacetas y resoluciones', to: '/marco-legal', color: 'bg-emerald-600' },
              { icon: BarChart3, label: 'Rendición de Cuentas', desc: 'Transparencia pública', to: '/institucion', color: 'bg-amber-600' },
              { icon: Users, label: 'Atención Ciudadana', desc: 'Contacto y denuncias', to: '/contacto', color: 'bg-purple-600' },
            ].map((item) => (
              <Link
                key={item.label}
                to={item.to}
